...
def save_model_as_onnx(model, device, ite_num, input_tensor_size=(1, 3, MAIN_SIZE, MAIN_SIZE)):
    ...
    onnx_file_name = f"dlmv_saved_models/{ite_num}.onnx"
    torch.onnx.export(
        model,
        x,
        onnx_file_name,
...
def save_checkpoint(state, filename="dlmv_saved_models/checkpoint.pth.tar"):
    """
...
def load_checkpoint(net, optimizer, scaler, filename="dlmv_saved_models/checkpoint.pth.tar"):
    """
...
def main():
    ...
    net = DeepLabV3MobileNetV3(num_classes=OUT_CHANNELS)
    net.to(device)
    net.train()
