...
def load_dataset(img_dir, lbl_dir, ext):
    ...
    """
    img_list = [img_dir + os.path.sep + x for x in os.listdir(img_dir)][:1134]
    lbl_list = [lbl_dir + os.path.sep + x for x in os.listdir(lbl_dir)][:1134]
...
def main():
    ...
    net = StraightU2Net(in_ch=3, out_ch=1, base_ch=32, mid_ch=16, num_blocks=2, dropout=0.05)
    net.to(device)
    net.train()

    optimizer = bnb.optim.AdamW8bit(
        net.parameters(), lr=3e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-2
    )
